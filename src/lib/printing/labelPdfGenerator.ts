...
const getDocumentStyles = () => `
  <style>
    /* Fuentes Gilroy */
    @font-face { font-family: 'Gilroy'; src: url('/font/Gilroy-Regular.ttf') format('truetype'); font-weight: 400; }
    @font-face { font-family: 'Gilroy'; src: url('/font/Gilroy-Medium.ttf') format('truetype'); font-weight: 500; }
    @font-face { font-family: 'Gilroy'; src: url('/font/Gilroy-SemiBold.ttf') format('truetype'); font-weight: 600; }
    @font-face { font-family: 'Gilroy'; src: url('/font/Gilroy-Bold.ttf') format('truetype'); font-weight: 700; }
    @font-face { font-family: 'Gilroy'; src: url('/font/Gilroy-ExtraBold.ttf') format('truetype'); font-weight: 800; }
    @font-face { font-family: 'Gilroy'; src: url('/font/Gilroy-Black.ttf') format('truetype'); font-weight: 900; }
    @font-face { font-family: 'Gilroy Light'; src: url('/font/Gilroy-Light.ttf') format('truetype'); font-weight: 300; }
    ...
    .label-element { position: absolute; box-sizing: border-box; overflow: hidden; }
...
const generateAndInjectScripts = (container: HTMLElement) => {
  // Generate barcodes
  container.querySelectorAll('svg[data-barcode-value]').forEach(svg => {
    ...
    if (value && window.JsBarcode) {
      try {
        JsBarcode(svg as SVGElement, value, {
          format,
          displayValue: true,
          height,
          width,
        ...
        svg.innerHTML = `<text style="font-family: monospace; font-size: 10px;">${escapeHtml(value)}</text>`;
      }
    }
  });

  // Generate QR codes
  container.querySelectorAll('canvas[data-qr-value]').forEach(canvas => {
    const value = canvas.getAttribute('data-qr-value');
    const size = parseInt(canvas.getAttribute('width') || '50');

    if (value && window.QRCode) {
      try {
        window.QRCode.toCanvas(canvas as HTMLCanvasElement, value, {
          width: size,
          margin: 0,
...
export const generateLabelPdf = async (
  ...
  for (let i = 0; i < expandedItems.length; i++) {
    ...
    container.innerHTML = getDocumentStyles();
      ...
      <body>
        ...
        <script>
          document.addEventListener('DOMContentLoaded', () => {
            ...
            if (container) {
              (${generateAndInjectScripts.toString()})(container);
            }
          });
        </script>
      </body>
