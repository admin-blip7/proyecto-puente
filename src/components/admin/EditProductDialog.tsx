...
export default function EditProductDialog({ isOpen, onOpenChange, product, onProductUpdated }: EditProductDialogProps) {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setValue("optimizedImage", undefined); // Reset optimized image if user selects a new one
      };
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({
        ...
        description: `El producto "${values.name}" ha sido guardado.`,
      });
      onOpenChange(false);
    } catch (error) {
                  ...
                  <div className="space-y-4 rounded-lg border bg-muted/50 p-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-center w-full">
                            <label htmlFor="dropzone-file-edit" className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-lg cursor-pointer bg-card hover:bg-muted relative">
                                {imagePreview ? (
                                    <>
